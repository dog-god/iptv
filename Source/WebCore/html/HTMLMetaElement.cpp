...
#endif
#if PLATFORM(ARMITE)
    if (equalIgnoringCase(name(), "page-view-size")) {
        if (!document()->frame()->ownerElement() ||
            document()->frame()->ownerElement()->hasTagName(frameTag)) {
            Vector<String> sizeArray;
            contentValue.string().split("*", sizeArray);
            if (sizeArray.size() == 2) {
                IntRect size(0,0,1280,720);
                size.setWidth(sizeArray[0].toInt());
                size.setHeight(sizeArray[1].toInt());
                WebView::getStaticInstance()->webViewPrivate()->setWebRect(size);
            }
        }
    }
#endif
