...
#if PLATFORM(ARMITE)
        if (m_classId.startsWith("ipanel:", false))
            m_serviceType = m_classId.substring(7);
#endif
        if (renderer())
            setNeedsWidgetUpdate(true);
...
#if PLATFORM(ARMITE)
        if (equalIgnoringCase(name, "jar")) {
            String value = document()->completeURL(p->value()).string();
...
#endif
#if PLATFORM(ARMITE)
    if (classId().startsWith("ipanel:", false))
        return true;
#endif
