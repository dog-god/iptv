...
#if PLATFORM(ARMITE)
    bool isProperty = Base::getOwnPropertySlot(thisObject, exec, propertyName, slot);
    if (!isProperty && WebViewPrivate::getPluginJSValue(thisObject, exec, propertyName, slot))
        return true;
    return isProperty;
#else
